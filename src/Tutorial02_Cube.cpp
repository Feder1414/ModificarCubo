...
void Tutorial02_Cube::CreateVertexBuffer()
{
    ...
    constexpr Vertex OctahedronVerts[6] =
    {
        {float3{ 0,  1,  0}, float4{1, 0, 0, 1}},  // 0 - Vértice superior (rojo)
        {float3{-1,  0, -1}, float4{0, 1, 0, 1}},  // 1 - Inferior izquierdo atrás (verde)
        {float3{ 1,  0, -1}, float4{0, 0, 1, 1}},  // 2 - Inferior derecho atrás (azul)
        {float3{ 1,  0,  1}, float4{1, 1, 0, 1}},  // 3 - Inferior derecho adelante (amarillo)
        {float3{-1,  0,  1}, float4{1, 0, 1, 1}},  // 4 - Inferior izquierdo adelante (magenta)
        {float3{ 0, -1,  0}, float4{0, 1, 1, 1}},  // 5 - Vértice inferior (cian)
    };

    BufferDesc VertBuffDesc;
    VertBuffDesc.Name      = "Octahedron vertex buffer";
    VertBuffDesc.Usage     = USAGE_IMMUTABLE;
    VertBuffDesc.BindFlags = BIND_VERTEX_BUFFER;
    VertBuffDesc.Size      = sizeof(OctahedronVerts);
    
    BufferData VBData;
    VBData.pData    = OctahedronVerts;
    VBData.DataSize = sizeof(OctahedronVerts);
    m_pDevice->CreateBuffer(VertBuffDesc, &VBData, &m_CubeVertexBuffer);
}

void Tutorial02_Cube::CreateIndexBuffer()
{
    // Índices para dibujar el octaedro con triángulos
    ...
    IndBuffDesc.Name      = "Octahedron index buffer";
